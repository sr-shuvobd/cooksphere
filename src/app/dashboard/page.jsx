  ...
  const fetchUserStats = async () => {
    ...
    try {
      ...
      const userRes = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/users/${encodeURIComponent(session.user.email)}`
      );
      if (userRes.ok) {
  ...
  useEffect(() => {
    const fetchAdminData = async () => {
      try {
        ...
        const recipesRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/recipes?limit=5`);
