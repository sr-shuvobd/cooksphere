...
export default function ReportsPage() {
  ...
  const fetchReports = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/reports`);
  ...
  const handleDismissReport = async (reportId) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/reports/${reportId}`, {
        method: "DELETE",
      });
      const data = await response.json();
  ...
  const handleDeleteRecipe = async (recipeId, reportId) => {
    ...
    try {
      // 1. Delete recipe
      const recipeResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/recipes/${recipeId}`, {
        method: "DELETE",
      });
      
      // 2. Dismiss report
      await fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/reports/${reportId}`, {
        method: "DELETE",
      });
