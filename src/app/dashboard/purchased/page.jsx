...
import { FaClock, FaUtensils, FaEye, FaLockOpen } from "react-icons/fa";
    ...
    const fetchPurchased = async () => {
      ...
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/users/${encodeURIComponent(session.user.email)}/purchased-recipes`
        );
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        {loading ? (
          <div className="py-20 flex flex-col items-center justify-center gap-4 bg-white dark:bg-[#03241f]/20 border border-stone-250 dark:border-white/5 rounded-3xl">
            ...
            <p className="text-sm font-bold text-stone-500 dark:text-stone-400">Loading purchased recipes...</p>
          </div>
        ) : recipes.length === 0 ? (
              ...
              >
                <div>
                  <div className="relative h-48 w-full bg-stone-100 dark:bg-[#021c17]">
                    ...
                    <span className="absolute top-4 right-4 bg-emerald-600 text-white text-[9px] font-black uppercase tracking-wider px-3 py-1.5 rounded-full flex items-center gap-1.5 shadow-md">
