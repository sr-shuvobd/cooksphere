...
export default function RecipeDetailsPage() {
    ...
    try {
      ...
      if (session?.user?.email) {
        const accessRes = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/recipes/${id}/purchase-status?email=${encodeURIComponent(
            session.user.email
          )}`
        );
        ...
        const favRes = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/recipes/${id}/favorite-status?email=${encodeURIComponent(
            session.user.email
          )}`
        );
        ...
        const likeRes = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/recipes/${id}/like-status?email=${encodeURIComponent(
            session.user.email
          )}`
        );
  ...
  const handleConfirmPurchase = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/recipes/${id}/purchase`, {
        method: "POST",
  ...
  const handleLikeToggle = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/recipes/${id}/like`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleReport = async () => {
    ...
    setReporting(true);
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/recipes/${id}/report`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: session.user.email }),
      });
